...
void SeplosBms::on_seplos_modbus_data(const std::vector<uint8_t> &data) {
  ...
  if (data.size() >= 44 && data[8] >= 8 && data[8] <= 16 && data[5] != 0x62) {
    this->on_telemetry_data_(data, false);
    return;
  } else if (data[5] == 0x62) {
    this->on_telemetry_data_(data, true);
    return;
  }
...
void SeplosBms::on_telemetry_data_(const std::vector<uint8_t> &data, bool current) {
  ...
  if (!current) {
    ...
    // float current = (float) ((int16_t) seplos_get_16bit(offset)) * 0.001f;
    // this->publish_state_(this->current_sensor_, current);

    // 52  56     0x13 0x54      Total battery voltage            4948 * 0.001f = 49.48          V
    ...
    // 54  58     0x39 0x5A      Residual capacity1                14682 * 0.01f = 146.82        Ah
    // 56  60     0x0A           Custom number                    10
    // 57  61     0x5B 0x68      Battery capacity1                 23400 * 0.01f = 234.00        Ah

    // 59  63      0x00 0x00                      Number of cycles                0
    this->publish_state_(this->charging_cycles_sensor_, (float) seplos_get_16bit(offset + 9) * 1.0f);

    // 61 capacité restante 3 octets Ah *0.001
    float residual_capacity = (float) ((uint16_t(data[offset + 11]) << 16) | (uint16_t(data[offset + 12]) << 8) |
                                       (uint16_t(data[offset + 13]) << 0)) *
                              0.001f;
    this->publish_state_(this->residual_capacity_sensor_, (float) residual_capacity);

    // 61  65     0x5B 0x68      Rated capacity                   23400 * 0.001f = 234.00        Ah
    float rated_capacity = (float) ((uint16_t(data[offset + 14]) << 16) | (uint16_t(data[offset + 15]) << 8) |
                                    (uint16_t(data[offset + 16]) << 0)) *
                           0.001f;
    this->publish_state_(this->rated_capacity_sensor_, (float) rated_capacity);

    if (data.size() < offset + 13 + 2) {
      return;
    }

    // 63  67       0x13 0x3A                    Port voltage            4922 * 0.001f = 49.22     V
    ...
    // this->publish_state_(this->state_of_charge_sensor_, (float) (residual_capacity * 100 / rated_capacity));
    this->send(0x61, this->pack_);

  } else if (current) {
    float current = (float) ((int16_t) seplos_get_16bit(8)) * 0.01f;
    this->publish_state_(this->current_sensor_, current);
    float total_voltage = (float) ((uint16_t) seplos_get_16bit(6)) * 0.001f;
    this->publish_state_(this->total_voltage_sensor_, total_voltage);
    float power = total_voltage * current;
    this->publish_state_(this->power_sensor_, power);
    this->publish_state_(this->charging_power_sensor_, std::max(0.0f, power));               // 500W vs 0W -> 500W
    ...
    // float residual_capacity = (float) ((uint16_t(data[offset + 11]) << 16) | (uint16_t(data[offset + 12]) << 8) |
    // (uint16_t(data[offset + 13]) << 0))*0.001f;
    this->publish_state_(this->state_of_charge_sensor_, data[10]);
  }
}
