  ...
  switch (event) {
    ...
    case ESP_GATTC_SEARCH_CMPL_EVT: {
      ...
      if (char_notify == nullptr) {
        ESP_LOGE(TAG, "[%s] No notify service found at device, not an Seplos v2 BMS..?", ADDR_STR(ADDR_STR(this->parent_->address_str())));
        break;
      }
      ...
      if (char_command == nullptr) {
        ESP_LOGE(TAG, "[%s] No control service found at device, not an BASEN BMS..?", ADDR_STR(ADDR_STR(this->parent_->address_str())));
        break;
      }
